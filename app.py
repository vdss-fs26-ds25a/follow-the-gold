...
.chapter-badge {
    ...
    padding: 4px 12px;
    border-radius: 20px;
    border: 1px solid rgba(230,199,90,0.18);
    margin-bottom: 12px;
}
...
.insight-box {
    background: rgba(230,199,90,0.04);
    border-left: 3px solid #d4b54c;
...
This project investigates global precious metals trade flows across 193 countries between 1988 and 2024 using bilateral trade and inflation data.
""")

st.markdown(
    "<small style='color:#666;'>Pre-2017 trade data: UN Comtrade HS 7106–7112 · Post-2016: BACI CEPII HS17 V202601</small>",
    unsafe_allow_html=True
)
...
if map_metric == "Share of Global Total (%)":
    country_totals["value"] = (
        country_totals["trade_value_usd"] /
        country_totals["trade_value_usd"].sum()
    ) * 100
    color_label = "Share (%)"
else:
...
Source: UN Comtrade + BACI CEPII aggregated by importing country.
The choropleth emphasizes spatial concentration patterns rather than exact quantitative comparison.
</div>
""", unsafe_allow_html=True)

st.markdown("""
<div class="insight-box">
Large economies such as China, India, and Hong Kong dominate by absolute value. 
Switching to percentage share highlights Switzerland's disproportionate role relative to its population size.
</div>
""", unsafe_allow_html=True)
...
fig2.add_trace(go.Scatter(
    ...
    mode="lines+markers",
    line=dict(color="#d4b54c", width=3),
    marker=dict(size=5),
    name="Gold Imports"
))
...
st.markdown("""
<div class="insight-box">
Gold trade activity increases during major crisis periods including the 2008 financial crisis, COVID-19, and the 2022 inflation wave. 
The pattern is broadly consistent with safe-haven behavior.
</div>
""", unsafe_allow_html=True)
...
scatter_data = gold_import[
    gold_import["year"] == scatter_year
].groupby("country")["trade_value_usd"].sum().reset_index()

scatter_data.columns = ["country", "trade_value"]

scatter_extra = df[df["year"] == scatter_year][
    ["country", "Headline CPI"]
].drop_duplicates()

scatter_merged = scatter_data.merge(
    scatter_extra,
    on="country",
    how="inner"
)
...
corr = scatter_merged["Headline CPI"].corr(
    np.log10(scatter_merged["trade_value"])
)
...
with m2:
    st.metric("Relationship Strength", strength.title())
...
fig_scatter = px.scatter(
    ...
    labels={
        ...
        "trade_value": "Gold Imports (log USD)"
    },
...
st.markdown("""
<div class="insight-box">
Surprisingly, inflation alone explains relatively little variation in gold import activity. 
The relationship is generally weak and differs across economy types, suggesting that geopolitical factors, industrial demand, and financial market dynamics also influence gold trade flows.
</div>
""", unsafe_allow_html=True)

st.markdown("""
<div class="caveat-box">
⚠️ Correlation does not imply causation. Trade volumes reflect multiple overlapping forces beyond inflation alone.
</div>
""", unsafe_allow_html=True)
...
fig3 = px.line(
    ...
    title="Trade Value Over Time by Precious Metal",
    color_discrete_map={
        "Gold": "#d4b54c",
        "Silver": "#bfc5cf",
        "Platinum": "#d9d7d2"
...
st.markdown("""
<div class="insight-box">
Gold consistently dominates global precious metals trade by value. 
While Silver and Platinum also react to crisis periods, Gold's response appears substantially larger and more persistent over time.
</div>
""", unsafe_allow_html=True)
...
st.markdown("## Switzerland as a Refining Hub")
...
global_gold = gold_import.groupby("year")[
    "trade_value_usd"
].sum().reset_index()
...
swiss_time = swiss_data.groupby("year")[
    "trade_value_usd"
].sum().reset_index()

swiss_vs_global = swiss_time.merge(
    global_gold,
    on="year",
    how="left"
)

swiss_vs_global["share"] = (
    swiss_vs_global["trade_value_usd"] /
    swiss_vs_global["global_total"]
) * 100
...
fig4 = px.line(
    ...
    color_discrete_sequence=["#d4b54c"]
)
...
st.markdown("""
<div class="insight-box">
Switzerland's share of global gold trade increases noticeably during crisis periods, suggesting that its refining and redistribution role intensifies when global demand for gold rises.
</div>
""", unsafe_allow_html=True)
...
with c1:
    ...
    <div class="metric-card">
        ...
        Inflation alone explains relatively little variation in gold imports
        </div>
    </div>
    """, unsafe_allow_html=True)
...
with c2:
    ...
    <div class="metric-card">
        ...
        <div class="metric-insight">
        Gold trade volume substantially exceeds other precious metals
        </div>
    </div>
    """, unsafe_allow_html=True)
...
with c3:
    ...
    <div class="metric-card">
        ...
        <div class="metric-insight">
        Switzerland becomes more prominent during crisis periods
        </div>
    </div>
    """, unsafe_allow_html=True)
...
st.markdown("""
<div class="insight-box">
The analysis provides evidence broadly consistent with gold's safe-haven role during periods of economic uncertainty. 
...
st.markdown("""
<div class="caveat-box">
⚠️ Limitations: The analysis relies on trade-flow data rather than investor-level behavior or ETF flows. 
...
Data Sources: UN Comtrade HS 7106–7112 · BACI CEPII HS17 · World Bank Global Inflation Database · VDSS FS2026
</small>
""", unsafe_allow_html=True)